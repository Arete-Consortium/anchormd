...
_CMDF_SALT = PRODUCT_KEY_CONFIG["claudemd-forge"][1]
...
class TestMultiProduct:
    ...
    def test_cross_product_checksum_fails(self) -> None:
        ...
        key = generate_key("agent-lint")
        assert not validate_key_checksum(key, "claudemd-forge")
        assert not validate_key_format(key, "claudemd-forge")
...
class TestValidateKeyFormat:
    ...
    def test_unknown_product_returns_false(self) -> None:
        assert validate_key_format("CMDF-ABCD-EFGH-54EF", "nonexistent") is False
...
class TestCheckSegment:
    ...
    def test_matches_cli_implementation(self) -> None:
        """Verify server key_gen matches CLI licensing.py for claudemd-forge."""
